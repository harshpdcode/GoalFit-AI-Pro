...
def create_schema():
    try:
        ...
        if not conn:
            ...
            cursor = conn.cursor()
            print("Connected to database for schema initialization.")
        ...
        CREATE TABLE IF NOT EXISTS professional_coaching_packages (
            ...
            price FLOAT,
            description TEXT,
            breakfast_meal_id INT NULL,
            lunch_meal_id INT NULL,
            dinner_meal_id INT NULL,
            snack_meal_id INT NULL,
            coach_notes TEXT,
            cover_image VARCHAR(255) DEFAULT 'static/images/packages/default_pkg.jpg',
            created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
            FOREIGN KEY (professional_id) REFERENCES professionals(id) ON DELETE CASCADE
        );
        """)
